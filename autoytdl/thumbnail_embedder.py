...
def embed_mp3(temp_dir_path, filename):
    ...
    if Path(temp_dir_path + "/"+filename[:-4]+".jpg").exists():
        ...
        Path(temp_dir_path + "/"+filename).replace(Path(temp_dir_path +
                                                        "/"+filename[:-4]+".temp.mp3"))
        os.system("ffmpeg -i \"" + str(Path(temp_dir_path + "/" + filename[:-4]+".temp.mp3")) + "\" -i \"" + str(Path(temp_dir_path + "/" + filename[:-4]+".jpg"))+"\"" +
                  " -v 0 -y -map 0:0 -map 1:0 -codec copy -id3v2_version 3 -metadata:s:v title=\"Album cover\" -metadata:s:v comment=\"Cover (front)\" \"" + str(Path(temp_dir_path + "/" + filename)) + "\"")
...
def embed_opus(temp_dir_path, filename):
    ...
    if Path(temp_dir_path + "/"+filename[:-5]+".webp").exists():
        os.system("ffmpeg -i \"" + str(Path(temp_dir_path + "/" + filename[:-5] + ".webp\"")) + " \"" + str(
            Path(temp_dir_path + "/" + filename[:-5] + ".jpg"))+"\"  -v 0 -y")
    ...
    if Path(temp_dir_path + "/"+filename[:-5]+".jpg").exists():
        ...
        os.system("kid3-cli -c 'set picture:\"in.jpg\"" +
                  " \"desc\"' \"" + str(Path(temp_dir_path + "/" + "in.opus"))+"\"")
        Path(temp_dir_path + "/" +
